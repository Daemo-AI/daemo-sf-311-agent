...
export class NIBRSCrimeFunctions {
  ...
  @DaemoFunction({
    description:
      "Search for law enforcement agency metadata (names, ORIs, locations). Use this ONLY to find agency ORI identifiers for a specific city/county - NOT for counting agencies or aggregating by state. CRITICAL: This function has pagination limits (max 10000 results per call) and returns partial results. To count how many agencies exist per state, you MUST use Promise.all() to call this function for ALL state codes IN PARALLEL (NOT sequentially in a loop). For crime statistics or comparisons, use getIncidentCounts instead. EXAMPLE for counting agencies by state: const results = await Promise.all(states.map(async (state) => { const data = await daemo.nibrs_crime_service.searchAgencies(state, undefined, undefined, undefined, undefined, 50000); return { state, count: data?.agencies?.length ?? 0 }; }));",
  ...
  @DaemoFunction({
    description:
      "Get crime incident counts, rates, and comparisons from NIBRS data. This is the PRIMARY function for: (1) Comparing crime across agencies/cities/departments - use groupBy:'agency', (2) Getting homicide/murder counts by agency - use offenseCode:'09A' with groupBy:'agency', (3) Comparing crime rates between states - use groupBy:'state', (4) Ranking agencies by crime type - use groupBy:'agency' with offenseCode, (5) Year-over-year agency trends - use groupBy:'agency_year'. Returns aggregated counts that can be used to calculate crime rates. PERFORMANCE TIP: When comparing multiple specific agencies (e.g., 5 cities), use Promise.all() to fetch each agency's data in parallel: const results = await Promise.all(oris.map(ori => daemo.nibrs_crime_service.getIncidentCounts({ ori, groupBy: 'offense', fromYear: 2025, toYear: 2025 })));",
    tags: [
      "nibrs",
      "incidents",
      "counts",
