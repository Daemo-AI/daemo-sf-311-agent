...
### Ask a Question (ask.sh)

For a better streaming experience with live output, use the included `ask.sh` script:

```bash
./ask.sh "<query>" "<max_tokens>" "<role>" "<analysis_mode>"
```

**Parameters:**
- `query` – Your question (required)
- `max_tokens` – Maximum tokens for the response (optional, defaults to 65536)
- `role` – The role to use
- `analysis_mode` – The analysis mode setting (boolean)

**Example:**
```bash
./ask.sh "How many 311 incidents are there by type?" "4096" "user" "true"
```

**Requirements:** [httpie](https://httpie.io/) and [jq](https://jqlang.github.io/jq/) must be installed.
